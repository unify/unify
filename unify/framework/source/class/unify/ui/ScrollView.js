...
members :
{
  ...
  _createElement : function()
  {
    ...
    // Add content element
    var contentElem = this.__contentElem = document.createElement("div");
    contentElem.className = "content";

    elem.appendChild(contentElem);

    var ScrollIndicator = unify.ui.ScrollIndicator;
    this.__horizontalScrollIndicator = new ScrollIndicator("horizontal");
    this.__verticalScrollIndicator = new ScrollIndicator("vertical");


    if(qx.core.Environment.get("qx.mobile.nativescroll")){
      this.setNativeScroll(true);
    } else {
      this.setNativeScroll(false);
    }
  ...
  _applyNativeScroll : function(value,old){
      var Registration = qx.event.Registration;
      var Style=qx.bom.element.Style;
      var elem=this.__element||this.getElement();
      var contentElem=this.__contentElem;
      var root = document.documentElement;
      if(value){
        Style.setStyles(contentElem,{"paddingTop":"1px","paddingBottom":"1px"});
        Style.setStyles(elem,{"WebkitOverflowScrolling":"touch","overflowX":"scroll","overflowY":"scroll"});
        qx.bom.element.Class.add(elem,"scroll-view-native");
        if(old!==null){
          elem.removeChild(this.__horizontalScrollIndicator.getElement());
          elem.removeChild(this.__verticalScrollIndicator.getElement());
          Registration.removeListener(elem, "touchstart", this.__onTouchStart, this);
          Registration.removeListener(contentElem, "transitionEnd", this.__onTransitionEnd, this);
          Registration.removeListener(root, "touchmove", this.__onTouchMove, this);
          Registration.removeListener(root, "touchend", this.__onTouchEnd, this);
          Registration.removeListener(root, "touchcancel", this.__onTouchEnd, this);
        }
        Registration.addListener(elem, "scroll",this.__onNativeScroll, this);
      } else {
        Style.setStyles(contentElem,{"paddingTop":"0px","paddingBottom":"0px"});
        Style.setStyles(elem,{"WebkitOverflowScrolling":"none","overflowX":"hidden","overflowY":"hidden"});
        qx.bom.element.Class.remove(elem,"scroll-view-native");
        if(old!==null){
          Registration.removeListener(elem, "scroll",this.__onNativeScroll, this);
        }
        elem.appendChild(this.__horizontalScrollIndicator.getElement());
        elem.appendChild(this.__verticalScrollIndicator.getElement());
  ...
  _applyScrollX : function(value){
    if(this.getNativeScroll()){
      qx.bom.element.Style.set(this.getElement(),'overflowX',value?"scroll":"hidden");
    } else {
      this._applyScrollMode(value);
    }
  },
  ...
  _applyScrollY : function(value){
    if(this.getNativeScroll()){
      qx.bom.element.Style.set(this.getElement(),'overflowY',value?"scroll":"hidden");
    } else {
      this._applyScrollMode(value);
    }
  },
  ...
  __scrollTo : function(left, top, animate)
  {
    if(this.getNativeScroll()){
      ...
      window.setTimeout(function(){
        var maxScrollTop=elem.scrollHeight-elem.clientHeight-1;
        if(maxScrollTop>-1){
          var newTop=Math.min(Math.max(top,1),Math.max(maxScrollTop,1));
  ...
  __onNativeScroll : function(e){
    var elem=this.__element;
    var scrollTop=elem.scrollTop;
    var scrollLeft=elem.scrollLeft;
    var maxScrollTop=elem.scrollHeight-elem.clientHeight;
    var maxScrollLeft=elem.scrollWidth-elem.clientWidth;
    if(maxScrollTop > 0 &&scrollTop==0){
      elem.scrollTop=this.__scrollTop=1;
    } else if(maxScrollTop>1&&scrollTop==maxScrollTop){
        elem.scrollTop=this.__scrollTop=maxScrollTop-1;
    }
    if(e){
      this.__scrollTop=scrollTop>1?(scrollTop<maxScrollTop?scrollTop:maxScrollTop):1;
      this.__scrollLeft=scrollLeft>0?(scrollLeft<maxScrollLeft?scrollLeft:maxScrollLeft):0;
      if (this.__hasScrollListener) {
        this.fireEvent("scroll");
      }
    }
